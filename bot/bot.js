...
app.get('/EuDepozitezIteme827192728292739127/', function (req, res) {
	...
	offers.makeOffer({
		...
		itemsFromMe: [],
		message: 'CSGOCROWN.GG | Deposit | +'+checksum+' coins | ['+code+']'
	}, function(err, r) {
		if(err) {
			logger.error('Error.');
			logger.debug(err);
...
app.get('/EuTrimitTradePentruAScoateIteme291729271889292919272819/', function (req, res) {
	if(ips[req.connection.remoteAddress]+15 >= time()) {
	return;
	} else {
	ips[req.connection.remoteAddress] = time();
	...
	offers.loadMyInventory({
		...
		if(err) {
			...
			offers.makeOffer({
				...
				message: 'CSGOCROWN.GG | Withdraw | -'+checksum+' coins | ['+code+']'
			}, function(err, r) {
				if(err) {
...
var ips = {};
app.get('/EuVerificTradeul/', function (req, res) {
	if(ips[req.connection.remoteAddress]+15 >= time()) {
		return;
	} else {
		ips[req.connection.remoteAddress] = time();
	...
	offers.getOffer({
		...
		if(err) {
			logger.error('Error loading trade offer');
			logger.debug(err);
			res.json({
				success: false,
				error: err.toString()
			...
			logger.debug(trade);
			var tmsg = trade.response.offer.message;
			if(trade.response.offer.trade_offer_state == 3) {
				if(tmsg.indexOf('Withdraw') > -1)
				{
					res.json({
						success: true,
						action: 'accept',
						result: 'Items received'
					});
				}
				else{
					res.json({
						success: true,
						action: 'accept',
						result: 'Coins received'
					});
				}
			} else if(trade.response.offer.trade_offer_state == 7) {
				res.json({
					success: true,
					result: 'Trade offer declined',
					action: 'cross'
				});
			} else if(trade.response.offer.trade_offer_state == 8) {
				res.json({
					success: true,
					result: 'Invalid items.',
					action: 'cross'
				});
			} else if(trade.response.offer.trade_offer_state == 1) {
				res.json({
					success: true,
					result: 'Invalid exception.',
					action: 'cross'
				});
			} else if(trade.response.offer.trade_offer_state == 2) {
				res.json({
					success: false,
					result: 'You need to accept the trade.',
					action: 'cross'
				});
			} else if(trade.response.offer.trade_offer_state == 11) {
				res.json({
					success: true,
					result: 'Items are in trade hold (come back in 15 days and you will recive your coins).',
					action: 'cross'
				});
			} else if(trade.response.offer.trade_offer_state == 10) {
				res.json({
					success: true,
					result: 'The offer is visible, but you or bot cancelled the trade.',
					action: 'cross'
				});				
			} else {
				res.json({
					success: false,
					error: 'You did not accept the trade offer'
				});
			}
		}
	});
...
function login(err, sessionID, cookies, steamguard) {
	...
	community.getWebApiKey('csgocrown.gg', webApiKey);
