...
function plus(user, socket) {
	query('SELECT * FROM `users` WHERE `steamid` = '+pool.escape(user.steamid), function(err, row) {
		if(err) return;
		if(time() > row[0].plus) {
			query('UPDATE `users` SET `plus` = '+pool.escape(time()+43200)+', `balance` = `balance` + 50 WHERE `steamid` = '+user.steamid);
			socket.emit('message', {
				type: 'alert',
				alert: 'Your new balance is: '+pool.escape(row[0].balance+50)
			});
			getBalance(user, socket);
		} else {
			socket.emit('message', {
				type: 'error',
				error: 'You need to wait '+(row[0].plus-time())+'s to receive 50 coins again.'
			});			
		}
	});
}
	...
	if(m.msg) {
		...
		if (res = /^\/send ([0-9]*) ([0-9]*)/.exec(m.msg)) {
			...
			if(user.rank > 0) {
				...
				socket.emit('message', {
					type: 'alert',
					alert: 'You muted '+res[1]+' for '+res[2]+' second(s).'
				});
			}
		} else {
			...
			query('SELECT `mute` FROM `users` WHERE `steamid` = '+pool.escape(user.steamid), function(err, row) {
				...
				if(row[0].mute > time()) {
					socket.emit('message', {
						type: 'alert',
						alert: 'You are muted for '+(row[0].mute-time())+' second(s).'
					});
					return;
				}
				query('SELECT `totalbets` FROM `users` WHERE `steamid`='+pool.escape(user.steamid), function(err,row) {
					if(err) return;
					
					if(row[0].totalbets > mincoinstochat) {
						io.sockets.emit('message', {
							type: 'chat',
					...
					if(row[0].totalbets < mincoinstochat) {
						socket.emit('message', {
							type: 'error',
							error: 'Total bet required to chat: '+pool.escape(row[0].totalbets)+'/'+mincoinstochat
						});
						return;
					}
				});
...
function setBet(m, user, socket) {
	...
	if(pause) {
		socket.emit('message', {
			type: 'error',
			enable: false,
			error: 'Drawing has already begun.'
		});
		return;
	}
		...
		if(row[0].balance >= m.amount) {
			query('UPDATE `users` SET `balance` = `balance` - '+parseInt(m.amount)+' WHERE `steamid` = '+pool.escape(user.steamid), function(err2, row2) {
				...
				query('INSERT INTO `bets` SET `user` = '+pool.escape(user.steamid)+', `amount` = '+pool.escape(m.amount)+', `lower` = '+pool.escape(m.lower)+', `upper` = '+pool.escape(m.upper), function(err3, row3) {
					...
					logger.debug('Bet #'+row3.insertId+' Amount: '+m.amount);
					query('UPDATE `users` SET `totalbets`= `totalbets` + '+pool.escape(m.amount)+' WHERE `steamid`='+pool.escape(user.steamid));
					query('UPDATE `users` SET `twithdraw`= `twithdraw` + 1 WHERE `steamid`='+pool.escape(user.steamid));
					checkTimer();
				})
			});
		} else {
...
function load() {
	...
	query('SELECT `id` FROM `rolls` ORDER BY `id` DESC LIMIT 1', function(err, row) {
		if((err) || (!row.length)) {
			logger.error('Error: Cant get number from the last game');
			logger.debug(err);
			process.exit(0);
			return;
		}
	...
	loadHistory();
	setTimeout(function() { io.listen(8080); }, 3992);
	checkTimer();
}
function loadHistory() {
	query('SELECT * FROM (SELECT id, roll FROM `rolls` WHERE roll <> -1 ORDER BY `id` DESC LIMIT 10) AS sub ORDER BY `id` ASC', function(err, row) {
		if(err) {
			logger.error('Cant get the number of last roll.');
			logger.debug(err);
			process.exit(0);
		}
		logger.trace('Rolls loaded with success.');
